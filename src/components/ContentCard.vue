  ...
  <div v-else class="lg:mx-auto m-2 lg:w-3/5 p-4 flex bg-gray-100 dark:bg-gray-300 rounded-lg">
    ...
    <div class="lg:pl-5 ml-2 lg:ml-5">
      <div class="lg:text-lg">
        <a class="text-black font-bold text-sm lg:text-lg hover:text-black hover:underline" :href="responseObject.url"
          target="_blank">{{ title }}</a>
        <span v-if="responseObject?.url" class="hidden lg:inline-block text-sm"> ( {{ domain }} )</span>
      </div>
      by
      <span class="cursor-pointer text-blue-700 font-bold text-sm lg:text-lg"
        @click="router.push(`/user/${username}`)">{{ username }}</span>
      <span v-if="!isJob">
        |
        <RouterLink class="text-black hover:text-black hover:underline text-sm lg:text-lg"
          :to="`/news/${props.id}/details`">{{ numberOfComments }} comments</RouterLink>
      </span>
